# Requirements: Игрок.Реальность

## Registration & Onboarding

| ID | Requirement | Priority |
|----|-------------|----------|
| REG-01 | `/start` command handler | v1 |
| REG-02 | Collect Telegram profile (first/last name, username) | v1 |
| REG-03 | Consent request (PDn + reports) | v1 |
| REG-04 | Consent required before proceeding | v1 |
| REG-05 | Archetype test (4 questions) | v1 |
| REG-06 | Archetype assignment (Head/Shell/Whirlwind/Ghost) | v1 |
| REG-07 | User profile creation | v1 |
| REG-08 | Referral link tracking | v1 |

## Quest Delivery

| ID | Requirement | Priority |
|----|-------------|----------|
| QUEST-01 | Channel scroll at 08:00 Moscow time | v1 |
| QUEST-02 | Personalized scroll by archetype | v1 |
| QUEST-03 | Scroll with completion button | v1 |
| QUEST-04 | ARQ queued delivery | v1 |
| QUEST-05 | Graceful retry (3 attempts) | v1 |
| QUEST-06 | 90 scrolls pre-loaded | v1 |

## Quest Completion & Progress

| ID | Requirement | Priority |
|----|-------------|----------|
| COMP-01 | Button tap completion | v1 |
| COMP-02 | XP award | v1 |
| COMP-03 | Streak tracking | v1 |
| COMP-04 | Streak reset on miss | v1 |
| COMP-05 | User timezone streak calculation | v1 |
| COMP-06 | Basic leaderboard | v1 |
| COMP-07 | Real-time leaderboard updates | v1 |

## Payments & Access

| ID | Requirement | Priority |
|----|-------------|----------|
| PAY-01 | Payment initiation via bot | v1 |
| PAY-02 | Platega.io API integration | v1 |
| PAY-03 | Payment link to user | v1 |
| PAY-04 | Webhook callback | v1 |
| PAY-05 | CONFIRMED → grant channel access | v1 |
| PAY-06 | CANCELED → notify, allow retry | v1 |
| PAY-07 | CHARGEBACKED → revoke access | v1 |
| PAY-08 | One-time invite link | v1 |
| PAY-09 | Access revocation on refund | v1 |
| PAY-10 | Payment idempotency | v1 |
| PAY-11 | Mentor commission calculation | v1 |

## Referrals & Commission

| ID | Requirement | Priority |
|----|-------------|----------|
| REF-01 | Unique referral link | v1 |
| REF-02 | Referral tracking on /start | v1 |
| REF-03 | Referrer reward | v1 |
| REF-04 | Referee reward | v1 |
| REF-05 | Configurable commission % | v1 |
| REF-06 | Commission tracking per mentor | v1 |
| REF-07 | Manual commission payout | v1 |

## Admin Panel

| ID | Requirement | Priority |
|----|-------------|----------|
| ADM-01 | User list with search/filter | v1 |
| ADM-02 | User details | v1 |
| ADM-03 | Scroll CRUD | v1 |
| ADM-04 | Payments list | v1 |
| ADM-05 | Payment details | v1 |
| ADM-06 | Settings configuration | v1 |
| ADM-07 | Audit log | v1 |
| ADM-08 | Admin authentication | v1 |

## TZ Gap Closure

| ID | Requirement | Priority |
|----|-------------|----------|
| GAP-01 | Structured scroll content (5 sections: common, individual, ritual, habits, micromovements) | v1 |
| GAP-02 | Per-archetype individual task storage (scroll_archetype_tasks) | v1 |
| GAP-03 | Archetypes reference table | v1 |
| GAP-04 | Configured daily publish time (08:00, editable in admin) | v1 |
| GAP-05 | Channel publishing of daily scroll (in addition to private delivery) | v1 |
| GAP-06 | Player report attachment (text/photo/video) on completion | v1 |
| GAP-07 | Full /profile command (archetype, day, XP, streak, role, payment status, referral link) | v1 |
| GAP-08 | /myteam command for curators (group up to 10, streak + payment status) | v1 |
| GAP-09 | Groups model and role transitions with history | v1 |
| GAP-10 | Specialist role and supplementary quests | v1 |
| GAP-11 | Clans (create, aggregate progress, clan leaderboard) | v1 |
| GAP-12 | Configurable XP weights and streak bonuses at 7/30/90 days | v1 |
| GAP-13 | Prize fund (reserve % of stream income, distribution rules) | v1 |
| GAP-14 | Lichnaya Trope (master-chat report duplication) and notifications/reminders | v1 |

## Traceability

| Requirement | Phase | Status |
|-------------|-------|--------|
| REG-01 | Phase 2 | Pending |
| REG-02 | Phase 2 | Pending |
| REG-03 | Phase 2 | Pending |
| REG-04 | Phase 2 | Pending |
| REG-05 | Phase 2 | Pending |
| REG-06 | Phase 2 | Pending |
| REG-07 | Phase 2 | Pending |
| REG-08 | Phase 2 | Pending |
| QUEST-01 | Phase 3 | Pending |
| QUEST-02 | Phase 3 | Pending |
| QUEST-03 | Phase 3 | Pending |
| QUEST-04 | Phase 3 | Pending |
| QUEST-05 | Phase 3 | Pending |
| QUEST-06 | Phase 3 | Pending |
| COMP-01 | Phase 4 | Pending |
| COMP-02 | Phase 4 | Pending |
| COMP-03 | Phase 4 | Pending |
| COMP-04 | Phase 4 | Pending |
| COMP-05 | Phase 4 | Pending |
| COMP-06 | Phase 4 | Pending |
| COMP-07 | Phase 4 | Pending |
| PAY-01 | Phase 5 | Pending |
| PAY-02 | Phase 5 | Pending |
| PAY-03 | Phase 5 | Pending |
| PAY-04 | Phase 5 | Pending |
| PAY-05 | Phase 5 | Pending |
| PAY-06 | Phase 5 | Pending |
| PAY-07 | Phase 5 | Pending |
| PAY-08 | Phase 5 | Pending |
| PAY-09 | Phase 5 | Pending |
| PAY-10 | Phase 5 | Pending |
| PAY-11 | Phase 5 | Pending |
| REF-01 | Phase 6 | Pending |
| REF-02 | Phase 6 | Pending |
| REF-03 | Phase 6 | Pending |
| REF-04 | Phase 6 | Pending |
| REF-05 | Phase 6 | Pending |
| REF-06 | Phase 6 | Complete |
| REF-07 | Phase 6 | Complete |
| ADM-01 | Phase 7 | Complete |
| ADM-02 | Phase 7 | Complete |
| ADM-03 | Phase 7 | Complete |
| ADM-04 | Phase 7 | Complete |
| ADM-05 | Phase 7 | Complete |
| ADM-06 | Phase 7 | Complete |
| ADM-07 | Phase 7 | Complete |
| ADM-08 | Phase 7 | Complete |
| GAP-01 | Phase 9 | Complete |
| GAP-02 | Phase 9 | Complete |
| GAP-03 | Phase 9 | Complete |
| GAP-04 | Phase 9 | Complete |
| GAP-05 | Phase 9 | Complete |
| GAP-06 | Phase 9 | Complete |
| GAP-07 | Phase 9 | Complete |
| GAP-08 | Phase 9 | Complete |
| GAP-09 | Phase 9 | Complete |
| GAP-10 | Phase 9 | Complete |
| GAP-11 | Phase 9 | Complete |
| GAP-12 | Phase 9 | Complete |
| GAP-13 | Phase 9 | Complete |
| GAP-14 | Phase 9 | Complete |

**Coverage:** 61/61 v1 requirements mapped ✓
